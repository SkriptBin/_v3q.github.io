...
class SKEditorLikeApp:
    ...
    def _build_ui(self) -> None:
        ...
        self.text.bind("<Control-MouseWheel>", lambda e: "break")
    ...
    def _apply_theme(self) -> None:
        if self._is_dark:
            bg, fg, side, status_bg = "#1e1e1e", "#d4d4d4", "#252526", "#007acc"
        else:
            bg, fg, side, status_bg = "#ffffff", "#111111", "#f0f0f0", "#e5e5e5"

        self.text.configure(bg=bg, fg=fg, insertbackground=fg)
        self.line_numbers.configure(bg=side, fg="#888")
        self.status.configure(bg=status_bg, fg="#ffffff" if self._is_dark else "#111")
    ...
    def _confirm_discard_if_needed(self) -> bool:
        # Lightweight guard: ask before clearing non-empty editors.
        if self.text.get("1.0", "end-1c").strip():
            return messagebox.askyesno("Confirm", "Discard current content?")
        return True
    ...
    def open_file(self) -> None:
        ...
        try:
            ...
            self._on_text_changed()
            self.root.title(f"SKEditor-Like - {os.path.basename(path)}")
